<script>
  ...
  const deleteSoapstone = async (deletedId) => {
    try {
      await fetch(`${API}/soapstones/${deletedId}`, {
        method: 'DELETE',
      });
      soapstones = soapstones.filter(soapstone => soapstone._id !== deletedId);
    } catch (error) {
      console.log(error);
    }
  }
  ...
  const handleAdd = async (e) => {
    const newSoapstone = { body: e.detail, prompt: id };
    try {
  ...
  <table>
    ...
    {#each soapstones as soapstone(soapstone._id)}
      ...
      <td>{soapstone.body}</td>
        <td>
          <form on:submit|preventDefault={() => deleteSoapstone(prompt._id)}>
